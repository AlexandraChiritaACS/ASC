...
int compute_NoUnifiedMem(void) {
    ...
    host_a = (float *) malloc(NUM_ELEM * sizeof(*host_a));
    host_b = (float *) malloc(NUM_ELEM * sizeof(*host_b));
    
    // TODO-plain: Alocare memorie (GPU/VRAM)
    cudaMalloc(&device_a, NUM_ELEM * sizeof(*device_a));
    cudaMalloc(&device_b, NUM_ELEM * sizeof(*device_b));
